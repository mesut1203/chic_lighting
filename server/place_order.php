...
//1.get user info and store it in database
    ...
    // 2. issue new order and order info in database
    $order_id = $stmt->insert_id;




//3.get products cart (from session)
...
// 4. store each single item in order_item database
        $stmt1 = $conn->prepare("INSERT INTO order_items (order_id,product_id,product_name,product_image,product_price,user_id,order_date )
                    VALUE (?,?,?,?,?,?,?,?)");   
        $stmt1->bind_param("iissiiis", $order_id,$product_id, $product_name,$product_image,$product_quantity,$user_id,$order_date);             
...
// 6. inform user whether everything is fine
